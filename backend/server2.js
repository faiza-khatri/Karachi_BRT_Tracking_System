...
app.get('/api/find-route', (req, res) => {
  ...
  const steps = [];
  steps.push({ type: 'board',    instruction: `Board at ${GRAPH[from].name}` });
  for (let i = 1; i < path.length - 1; i++) {
    steps.push({ type: 'transfer', instruction: `Pass through ${GRAPH[path[i]].name}` });
  }
  steps.push({ type: 'exit', instruction: `Exit at ${GRAPH[to].name}` });
